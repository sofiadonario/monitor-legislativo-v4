              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    <span className="bg-blue-100 text-blue-800 px-2 py-1 text-xs rounded-full">
                      {selectedDocument.metadata.tipoDocumento}
                    </span>
                    <span className="bg-purple-100 text-purple-800 px-2 py-1 text-xs rounded-full">
                      {selectedDocument.metadata.autoridade}
                    </span>
                    <span className="bg-green-100 text-green-800 px-2 py-1 text-xs rounded-full">
                      {selectedDocument.metadata.localidade}
                    </span>
                  </div>
                </div>
                ...
                {selectedDocument.metadata.description && (
                  <div>
                    ...
                    <p className="text-gray-600">{selectedDocument.metadata.description}</p>
                  </div>
                )}

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <h4 className="font-medium text-gray-700 mb-1">URN:</h4>
                    <p className="text-sm text-gray-600 font-mono break-all">
                      {selectedDocument.metadata.urn}
                    </p>
                  </div>
                  <div>
                    <h4 className="font-medium text-gray-700 mb-1">Date:</h4>
                    <p className="text-sm text-gray-600">
                      {new Date(selectedDocument.metadata.date).toLocaleDateString('pt-BR')}
                    </p>
                  </div>
                </div>

                {selectedDocument.metadata.subject.length > 0 && (
                  <div>
                    <h4 className="font-medium text-gray-700 mb-2">Subjects:</h4>
                    <div className="flex flex-wrap gap-1">
                      {selectedDocument.metadata.subject.map((subject, index) => (
                        <span
                          key={index}
                ...
                <div className="flex gap-4 pt-4">
                  <a
                    href={selectedDocument.metadata.identifier}
                    target="_blank"
