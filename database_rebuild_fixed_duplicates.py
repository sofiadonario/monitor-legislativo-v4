#!/usr/bin/env python3
"""
DATABASE REBUILD WITH DUPLICATE HANDLING
This script handles duplicate URNs properly during database population
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('./data_current/processed/database_rebuild_fixed.log'),
        logging.StreamHandler()
    ]
)

def main():
    """Complete database rebuild process with duplicate handling"""
    print("🚀 STARTING DATABASE REBUILD WITH DUPLICATE HANDLING")
    print("="*80)
    ...
    try:
        ...
        # Step 2: Create new schema WITHOUT unique URN constraint
        ...
        -- Create main documents table (NO unique URN constraint)
        CREATE TABLE documents (
            id SERIAL PRIMARY KEY,
            titulo TEXT NOT NULL,
            urn VARCHAR(500),  -- Allow duplicates
            url TEXT,
            numero VARCHAR(200),
            ementa TEXT,
            assuntos TEXT,
        ...
        # Step 3: Create indexes (without unique URN constraint)
        ...
        CREATE INDEX idx_documents_urn ON documents(urn);  -- Regular index, not unique
        ...
        logging.info("🔧 Step 5: Preparing and inserting data...")
                ...
                try:
                    ...
                    def safe_int(int_val):
                        if pd.isna(int_val):
                            return None
                        try:
                            val = int(float(int_val))  # Handle float strings
                    ...
                    logging.warning(f"Skipping row {_}: {e}")
                    continue
            ...
            if batch_data:
                try:
                    with conn.cursor() as cur:
                        ...
                        logging.info(f"   Progress: {total_inserted:,}/{len(df):,} rows inserted...")
                except Exception as e:
