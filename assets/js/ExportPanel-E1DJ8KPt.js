...
import { j as jsxRuntimeExports } from "./index-B_2E3y_G.js";
...
const exportMapToPNG = async (options = {}) => {
  const {
    quality = 0.9,
    scale = 2,
...
const exportMapWithMetadata = async (documents, selectedState, options = {}) => {
  const {
...
class LocalCache {
  constructor(options) {
    __publicField(this, "prefix", "legislativo_");
    __publicField(this, "maxAge", 36e5);
    // 1 hour default
    __publicField(this, "maxSize", 5 * 1024 * 1024);
    // 5MB max
   ...
   */
  async getOrFetch(key, fetchFn, ttl = this.maxAge) {
...
const cacheUtils = {
   ...
   * Cache API response
   */
  async cacheAPIResponse(url, options, ttl = 9e5) {
...
const CACHE_CONFIG = {
  ...
  "/api/v1/proposals": { ttl: 72e5, priority: "high" },
  "/api/v1/sources": { ttl: 864e5, priority: "low" },
  "/api/v1/geography": { ttl: 2592e6, priority: "low" },
...
class CachedFetch {
  ...
  async fetch(url, options = {}) {
    ...
    if (!finalOptions.method || finalOptions.method === "GET") {
      ...
      if (cached !== null) {
        if ("headers" in cached && ((_a = cached.headers) == null ? void 0 : _a["X-Cache"])) {
    ...
    for (let i = 0; i <= retries; i++) {
      try {
        ...
        if (response.status >= 500 && i < retries) {
          await this.delay(Math.min(1e3 * Math.pow(2, i), 1e4));
          continue;
        }
        return response;
      } catch (error) {
...
const ExportPanel = ({
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      switch (exportFormat) {
        ...
        case "png":
          if (includeMetadata) {
            await exportMapWithMetadata(filteredDocuments, void 0, {
              format: "png",
              includeControls: false,
              includeLegend: true
            });
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "export-panel-overlay", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "export-panel", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "export-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "Exportar Dados" }),
